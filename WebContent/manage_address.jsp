...
<link rel="stylesheet" href="css/manage_address.css" type="text/css">
<!-- <script type="text/javascript">
	function addAddress(){
		var name=document.getElementById("username").value;
		//alert(name)
		window.location="addAddress?name="+name;
	}
</script> -->
	
<script type="text/javascript">
	var xmlHttpRequest;
	var name,phone,country,city,address,code;
	...
	function addAddress(){
		...
		fSend("POST","addAddress","name="+name+"&phone="+phone+"&country="+country+"&city="+city+"&address="+address+"&code="+code,fCallback);
	}
</script>
<script type="text/javascript">
	function modifyAddress(id){
		var form=document.getElementById("form");
		form.action="wholeAddress?method=modify&user_rec_id="+id;
		form.submit();
	}
	
	function deleteAddress(id){
		var form=document.getElementById("form");
		if(confirm('您确定删除这条收货地址吗？')){
			form.action="wholeAddress?method=delete&user_rec_id="+id;
			form.submit();
			return true;
		}else{
			return false;
...
<form action="" method="post" id="form">
  <table class="have">
    ...
    <c:forEach var="rec" items="${applicationScope.rec }">
      <tr>
        <td>
           <div class="name" id="rname"><span>${rec.rec_name }</span></div>
        </td> 
        <td>${rec.province }省${rec.city }市${rec.address }</td>    
        <td>${rec.code }</td>
        <td>${rec.rec_phone }</td>
        <td>
            <div class="operation">
                <a href="javascript:modifyAddress(${rec.user_rec_id })">修改</a>&nbsp;|&nbsp;
                <a href="javascript:deleteAddress(${rec.user_rec_id })">删除</a>&nbsp;|&nbsp;
            </div>
        </td>
      </tr> 
      </c:forEach>  
    ...
    <table class="add" border="1">
        <tr>
            ...
            <td><input class="input-name" type="text" id="username" placeholder="长度不超过25个字符" maxlength="25"></td>
        </tr>
        <tr>
            <td class="recieve_phone">手机号码：*</td>
            <td><input class="input-phone" type="text" id="phone" placeholder="中国大陆手机号"></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <textarea class="input_detail_address" id="address" placeholder="填写详细收货地址"></textarea>
            </td>
        </tr>
        <tr>
            <td class="recieve_email">邮政编码：</td>
            <td><input class="input_email" id="code" type="text"></td>
        </tr>
        <tr>
